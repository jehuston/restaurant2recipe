...
class MyRecommender():
    '''
    A class that will build take in text documents, build a dictionary and
    index, and return recommendations from that index upon new input.
    '''
    def __init__(self, model):
        self.model = model
        self.stopset = set(stopwords.words('english'))
        # add some words that appear a lot in menu data
        self.stopset.update(['description', 'available'])
    ...
    def _prepare_documents(self, db):
        ...
        Given database connection, collect all recipe documents, join
        ingredients lists into strings, and return as array.
        '''
        cursor = db.recipes.find(
            {},
            {'rec_id': 1, 'title': 1, 'ingredients': 1, '_id': 0}
        )
    ...
    def _clean_text(self, documents):
        ...
        Given array of strings (recipes or restaurant menus), tokenize and
        return as array.
        '''
        ...
        for doc in documents:
            ...
            for word in words:
                ...
                if word not in self.stopset and not any(c.isdigit() for c in word):
    ...
    def _create_doc_vectors(self, text_array):  # word2vec only
        ...
        Translates words/tokens into word vectors, sums to create document
        vectors.
        '''
        doc_vectors = []
        unfound_words = 0
    ...
    def _vectorize_restaurant_menu(self, name, db):
        ...
        Given restaurant name that exists in database, return menu and
        vectorize to prepare for similarity query.
        '''
    ...
    def get_recommendations(self, name, db, num):
        '''
        INPUT: index (), menu vector (ARRAY), number (INT) of recommendations
            requested
        OUTPUT: dataframe/series(?) of recommended recipes

        Returns top n recommended recipes based on cosine similiarity to
        restaurant menu.
        '''
